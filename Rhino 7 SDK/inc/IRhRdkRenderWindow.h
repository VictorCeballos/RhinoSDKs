...
enum class ComponentOrder : int
{
	Irrelevant = 0x00, // Used in single-value channels.
	...
	RGBA = 0x01, // This is the default (to match Rhino 5).
	ARGB = 0x02,
	RGB  = 0x03,  // This will only access 3 components, even in the case of the RGBA channel.
	BGR  = 0x04,  // This will only access 3 components, even in the case of the RGBA channel.
	ABGR = 0x05,
	BGRA = 0x06,
...
public:
	...
	/*virtual*/ bool GetRenderChannels(OUT ON_SimpleArray<UUID>& aChan) const; // = 0; [SDK_UNFREEZE]
	...
	/** This function must be called when an asynchronous render process has ended.
		...
		\param rsc is rsc_completed for a successful render or canceled render,
	...
	public:
		...
		/** Set a rectangle of values into the channel. The input parameters are checked for validity.
			...
			\param co is the component order of the source data.
			\param pData contains the data to store in the channel starting at the specified position. There must be
			 the correct amount of data in the correct layout to match the shape of the rectangle and the stride.
			\return \e true if successful, else \e false. */
	...
	public:
		...
		/** Get value(s) from the channel.
			...
			\param pData is a buffer which must have enough room to accomodate the returned
			 values. Depending on the channel, this could be one or more floats.
			\return \e true if successful, else \e false. */
		...
		/** Get a rectangle of values from the channel. The input parameters are checked for validity.
			...
			\param co is the component order of the destination data.
			\param pData accepts the data from the channel starting at the specified position and it must have enough room to
			 accomodate the correct amount of data in the correct layout to match the shape of the rectangle and the stride.
			\return \e true if successful, else \e false. */
	...
	public:
		...
		virtual unsigned int TextureHandle(void) const = 0;
	...
	/** Use this utility method to automatically add wireframe channels to the rendered image.
		...
		\param region is the area of the rendering you want to display. This should match the size
	...
	/*virtual*/ void SetWatermarkText(const wchar_t* wsz); // = 0; [SDK_UNFREEZE] Uses EVF "SetWatermarkText" for now.

	/** Call this to clear the end time. Used when extending a rendering (e.g., in Cycles). */
	/*virtual*/ void ClearEndTime(void); // = 0; [SDK_UNFREEZE] Uses EVF "ClearEndTime" for now.

	/** This function is here to support raytraced mode. */
	/*virtual*/ void SetIsRendering(bool b); // = 0; [SDK_UNFREEZE] Uses EVF "SetIsRendering" for now.
...
public: // Events.
	...
	/**virtual*/ void RegisterEventWatcher(IEventWatcher& ew); /* = 0 */ // [SDK_UNFREEZE] Uses EVF "RegisterEventWatcher" for now.

	/** Call this to unregister an event watcher. */
	/**virtual*/ void UnregisterEventWatcher(IEventWatcher& ew); /* = 0 */ // [SDK_UNFREEZE] Uses EVF "UnregisterEventWatcher" for now.
...
public: // Internal expert methods for the RDK UI.
	...
	/*virtual*/ bool OnClose(void) const; // = 0; [SDK_UNFREEZE]
};
