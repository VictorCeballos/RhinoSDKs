...
*/
RHINO_SDK_FUNCTION
BOOL RhinoSelectLayerDialog(
  HWND hwndParent,
  const wchar_t* lpsPrompt,
  int& layer_index,
  BOOL bDisplayDialog = TRUE,
  BOOL bEnableNewLayerButton = FALSE,
  BOOL bEnableSetLayerCurrentButton = FALSE,
  BOOL* pbSetLayerCurrentButtonValue = nullptr
);
...
*/
RHINO_SDK_FUNCTION
BOOL RhinoSelectMultipleLayersDialog(
  HWND hwndParent,
  const wchar_t* lpsPrompt,
...
Remarks:
  ...
  results.  When curves are joined they are made into polycurves.
  Memory for the Brep is allocated and becomes the responsibility
  of the caller.
*/
RHINO_SDK_FUNCTION
BOOL RhinoMergeCurves(
...
*/
RHINO_SDK_FUNCTION
BOOL RhinoExtendLineThroughBox(
...
public:
  ...
  BOOL m_limit_range;
  //determine what layer to put output on
...
Parameters:
  degree - [in] The degree of the curve >=1.  Degree must be odd.
...
/*
Description:
  Subtract each Brep of Inbreps1 from each Brep of InBreps0. Separate.
Parameters:
  InBreps0       - [in]  first set of input Breps.
  InBreps1       - [in]  second set of input Breps.
  tolerance      - [in]  distance tolerance
...
/*
Description:
  Find Union of muiltiple planar faces.
Parameters:
  InRegions  - [in]  Array of Breps, each with a single face with a planar surface.
                     Will fail if any do not satisfy this requirement.
...
Parameters:
  ...
  InputInnerBoundaryCurves - [in] These polylines will create holes in the output mesh.
                                  ...
                                  mesh could be invalid (all faces created contained vertexes from the perimeter
                                  boundary).
                                  Optional, can be empty.
...
Description:
  ...
  or for use wit the RhinoOffsetCurveNormal() function.
Parameters:
  pSrf     [in] - Surface from which normals are calculated.
  pCrv     [in] - 3d curve lying on the surface along which normals are calculated.
  params   [in] - parameters in the domain of pCrv for offset points to interpolate
