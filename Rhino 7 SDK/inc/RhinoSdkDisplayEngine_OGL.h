...
public:
  ...
  CRhinoDisplayEngine_OGL(MRViewportView*);
#endif
...
public:
  ...
  void DrawGroundPlane(const class CDisplayPipelineAttributes& dpa, const class CRhGLGroundPlane& groundplane) override;
...
#pragma region V7 WIP display
  //not virtual; added during V7 and only called by display pipeline
...
private:
  ...
  void BufferShadedMeshes(class CRhOglMeshBuffer* buffer);
  ...
  void BufferHatch(CRhinoCacheHandle& cache, const ON_Color& fillColor, const ON_Xform* xform);
  void BufferHatch(class CRhHatchCacheItem& item, const ON_Xform* xform);
  void FlushHatchBuffer();
  void BufferText(class CRhTextCacheItem& item, const ON_Xform* xform);
  void FlushTextBuffer();

  void BufferFilledTriangles(int triangleCount, const ON_3fPoint* vertices, const ON_Color& color);
...
//          consisting of any alpha-numeric characters. The idea is that you choose 
