...
/** \class IRhRdkPostEffect
	...
	In the new system, post effects consist of 'early' post effects like Glare and Glow,
	'tone mapping' post effects and 'late' post effects like Gamma.
	...
	- The renderer produces a rendering in a frame buffer composed of channels. The channels are HDR capable.
	- The early post effect manager's post effects are executed to produce modified versions of some of these channels.
	...
	- If you want your post effect to be run before tone mapping, it should be an early post effect implemented by subclassing CRhRdkEarlyPostEffect.
	- If your post effect performs Tone Mapping, it should be a Tone Mapping post effect implemented by subclassing CRhRdkToneMappingPostEffect.
	- If you want your post effect to be run after tone mapping, it should be a late post effect implemented by subclassing CRhRdkLatePostEffect.
...
public:
	...
	{
		Never,    // The post effect does not support execution while rendering.
		Always,   // The post effect supports execution while rendering and it should be run every time the dib is updated.
		UseDelay, // The post effect supports execution while rendering but only after a delay the first time.
	};
	...
	/** Execute the post effect.
		\param pipeline provides access to the post-effect pipeline.
	...
	/** Read the state from document defaults. This is implemented by RDK to call ReadState()
		so usually a post effect only has to implement ReadState(). However, a post effect can override
		this method to take complete control of how the document defaults are read.
		\note If you override this method, you must \e not call the base class. */
	virtual bool ReadFromDocumentDefaults(const CRhinoDoc& doc) = 0;

	/** Write the state to document defaults. This is implemented by RDK to call WriteState()
		so usually a post effect only has to implement WriteState(). However, a post effect can override
		this method to take complete control of how the document defaults are written.
		\note If you override this method, you \e must call the base class because other information than
		 just the state is written to the document regarding the post effect's position and visibility. */
	virtual bool WriteToDocumentDefaults(CRhinoDoc& doc) const = 0;
...
public:
	...
	/** \return a list of the post effects to be executed by this pipeline in order. */
	...
	public:
		...
		/** Commit changes to the channel so that those changes can be used by subsequent post effects in the chain.
			...
			\param pChannel is a pointer to the channel to commit. */
...
public:
	...
	*/
	...
	/** Called from a post effect's Execute() method to run the post effect asynchronously.
		...
		\param job is the job that will be executed asynchronously (on a thread).
