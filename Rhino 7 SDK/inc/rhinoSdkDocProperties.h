...
/*
  class CRhinoAnnotationSettings 
    ...
    Description:
      Doc scope settings that apply to annotation objects
*/
// 3-19-03 LW changed to exported class

// March 22, 2010 - removed global dimstyle from Rhino 5
// Dimscale is now part of DimStyles and this class is obsolete
// 27 Aug, 2010 - Lowell - CRhinoAnnotationSettings replaced by CRhinoAnnotationSettingsEx
...
public:
  ...
  BOOL operator ==(const CRhinoAnnotationSettings& other);
  BOOL operator !=(const CRhinoAnnotationSettings& other);
...
private:
//  double m_dimscale;  Reused as hatch scale and text scale
  float m_text_scale;
  float m_hatch_scale;
};
...
public:
  ...
  double ModelSpaceTextScale() const;
  // Parameters:
  //    scale - [in]  Sets the scale factor for text in model space when Annotation Scaling is enabled
  void   SetModelSpaceTextScale(double scale);

  // 14 Jan 2011 - Added hatch scaling functions
  // Returns:
  //    The scale factor for hatches in model space when Hatch Scaling is enabled
  // See comments in ON_3dmAnnotationSettings for more details
  double ModelSpaceHatchScale() const;
  // Parameters:
  //    scale - [in]  Sets the scale factor for hatches in model space when Hatch Scaling is enabled
  void   SetModelSpaceHatchScale(double scale);

  // Returns: 
  //   true  if ModelSpace Annotation Scaling is enabled
  //   false if not.
  // If ModelSpaceAnnotationScaling is on, sizes in dimstyles are multiplied by 
  // dimstyle.m_dimscale when the annotation is displayed in a model space viewport not in a detail
  // 
  // See comments in ON_3dmAnnotationSettings for more details
  bool   IsModelSpaceAnnotationScalingEnabled() const;
  // Parameters:
  //   bEnable - [in]  Enables or disables Annotation Scaling
  void   EnableModelSpaceAnnotationScaling(bool bEnable);

  // Returns: 
  //   true  if LayoutSpace Annotation Scaling is enabled
  //   false if not.
  // If LayoutSpaceAnnotationScaling is on, sizes in dimstyles are multiplied by 
  // dimstyle.m_dimscale when the annotation is displayed in a detail viewport not in a detail
  // 
  // See comments in ON_3dmAnnotationSettings for more details
  ...
  // Returns: 
  //   true  if Hatch Scaling is enabled
  //   false if not.
  // See comments in ON_3dmAnnotationSettings for more details
  bool   IsHatchScalingEnabled() const;
  // Parameters:
  //   bEnable - [in]  Enables or disables Hatch Scaling
...
public:
  ...
  */
  ...
  /*
  */
  BOOL GetON_3dmAnnotationSettings( ON_3dmAnnotationSettings& settings) const;
  ...
  */
  ...
  //ON_DEPRECATED void SetAnnotationSettings( const CRhinoAnnotationSettings& as, BOOL bUpdateDims = TRUE);
  ...
  */
  ...
  See Also:
    ...
    CRhinoDocProperties::MoveNamedConstructionPlane
  */
  BOOL RemoveNamedConstructionPlane( int cplane_index );
  ...
  */
  ...
  See Also:
    ...
    CRhinoDocProperties::MoveNamedView
  */
  BOOL RemoveNamedView( int view_index );
...
private:
  ...
  // When the model is used in conjuntion with GIS data,
