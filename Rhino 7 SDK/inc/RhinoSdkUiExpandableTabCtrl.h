...
//
// Description:
//  Control class for tab items.  Used by CRhinoUiExpandableTabCtrl class bellow.
//
...
public:
  ...
  // Returns:
  //   Only meaningfull if CRhinoUiExpandableTabCtrl::SetImageList() called.  If so
...
public:
  ...
  //   pRC [in] Class to create and add to control.  Use RUNTIME_CLASS( CMyClass) macro to format this paramater.
  //            Class decloration must include 	DECLARE_DYNCREATE(CMyClass) macro.  Class implementation must
  //            inlcude IMPLEMENT_DYNCREATE(CMyClass, CRhinoUiDialog) macro.
  ...
  //              horizontal scroll bar infor is returned.
  //
  // Returns:
  ...
  //   
  // Description:
  //   Get client rect positioned releative to current scroll bar postion.
  //
  // Parameters:
...
protected:
  ...
  //
  // Returns:
  //   Retuns size reqired necessary to display all tab items.
  //
  // See Also:
