...
public:
  ...
  AFX_MODULE_STATE* DialogModuleState() const { return BASECLASS::m_pModuleState; }
  //---------------------------------------------------------------------------
#pragma region IRhinoWindow overrides
  ...
  ON_2iPoint Location() const override
  {
    ...
    auto parent = ::GetParent(BASECLASS::GetSafeHwnd());
    if (parent == NULL)
  ...
  {
    MFC_DIALOG_MODULE_STATE;
    return ::IsWindow(BASECLASS::GetSafeHwnd()) && BASECLASS::DestroyWindow();
  }
  //---------------------------------------------------------------------------
...
#pragma region IRhinoPage overrides
  ...
  {
    return BASECLASS::LoadIconResource(m_icon_resource_id, sizeInPixels);
  }
  //---------------------------------------------------------------------------
  // Description:
