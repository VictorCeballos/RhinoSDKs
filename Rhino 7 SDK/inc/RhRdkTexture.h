...
public: // Adjustment support - added 9th July 2008 ALB
	...
	public:
		...
		static DWORD CRC(IColorAdjuster* pca);
...
public:
	...
	/** Modify an CRhRdkSimulatedTexture to simulate this texture as best you can.
		...
		\param tex accepts the simulation.
	...
	/** Call this method to determine whether the texture should be treated as gamma packed (usually to 2.2)
		The default implementation returns \e false.  Bitmap textures will return true as long as they are not
		in the child slot designated as requiring linear workflow adjustment by the host material. See CRhRdkMaterial::TextureRequiresLinearWorkflowAdjustment*/
	virtual bool IsLinear(void) const;

	/** Call this method to determine if the texture appears to be a normal map. */
	/*virtual*/ bool IsNormalMap(void) const;
	...
	/** Call this method to determine the dimensions of the image in pixels. 
		...
		\return \e true if the image is pixel-based (i.e., a bitmap of some sort), else \e false.
		 In this case, the output values may not be filled in. */
...
public:
	/** Call this method to update all of the textures with a new filename for file based textures. 
		\param wszFilename is the new filename.
		\param pController is the controller of the UI if supported.
		\param wszChildSlot is the child slot name of the filename field.  "filename" is used if nullptr is passed. */
