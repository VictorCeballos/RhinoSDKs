...
/** \class IRhRdkNewContentCtrl -- AKA 'NCC'.
	...
	This is a replacement for CRhRdkContentCtrl and allows a UI to display
	a chosen content with a drop-down list of content thumbnails to choose from.
...
public:
	...
	/** \return The ids of the data sources needed to make the control work. */
	...
	/** Make the control open its drop-down thumbnail list (if closed).
		\param uReserved is reserved for future use and should be zero.
		\return \e true if successful, else \e false. */
...
/** \class CRhRdkNewContentCtrl -- AKA 'NCC'.
	...
	Using this class as a dialog member is no longer supported. Please call IRhRdkNewContentCtrl::New()
	and use the returned interface instead. You will need to call SetParent() and SetController().

*/
class RDK_DEPRECATED_CLASS CRhRdkNewContentCtrl
{
	// OBSOLETE
};
