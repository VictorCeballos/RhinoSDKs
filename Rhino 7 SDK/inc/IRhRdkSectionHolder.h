...
public:
	...
	/*virtual*/ void SetFullHeightSection(IRhinoUiSection& section); // = 0; [SDK_UNFREEZE] Implemented via EVF(L"SetFullHeightSection").
	...
	{
	public:
		/*virtual ~IIterator() { } // [SDK_UNFREEZE]*/
...
class RHRDK_SDK IRhRdkRenderSettingsSection
{
public:
	/** Get the current environment settings. */
	virtual const IRhRdkCurrentEnvironment* CurrentEnvironment(void) const = 0;

	/** Get the dithering settings. */
	virtual const IRhRdkDithering* Dithering(void) const = 0;

	/** Get the ground plane settings. */
	virtual const IRhRdkGroundPlane* GroundPlane(void) const = 0;

	/** Get the linear workflow settings. */
	virtual const IRhRdkLinearWorkflow* LinearWorkflow(void) const = 0;

	/** Get the Rhino settings. */
	virtual const IRhRdkRhinoSettings* RhinoSettings(void) const = 0;

	/** Get the skylight settings. */
	virtual const IRhRdkSkylight* Skylight(void) const = 0;
