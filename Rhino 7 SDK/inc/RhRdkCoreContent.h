...
public:
	...
	/** Create a preview of the content for use in the content editor thumbnail display.
		...
		\param pSceneServer A scene server that defines the scene for the preview rendering.  Can be NULL (in which case, a default scene is used)
		\param dibOut is a DIB containing the rendered image if the method returns \e true.
		\return \e true if successful, else \e false. */
	virtual bool CreatePreview(CRhRdkRenderPlugIn& plugIn, const ON_2iSize& sizeImage,
                               RhRdkPreviewQuality qual, const IRhRdkPreviewSceneServer* pSceneServer, IRhRdkPreviewCallbacks* pNotify, CRhinoDib& dibOut) const override;
