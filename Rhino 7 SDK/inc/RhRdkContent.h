...
{
public:
	/** Construction should not call any virtual content methods. If you need to do anything
	...
	{
		TreeControl    = 0, // Dynamic icon appears in a tree control.
		SubnodeControl = 1, // Dynamic icon appears in a sub-node control (\see CRhRdkSubNodeCtrl)
		ContentControl = 2, // Dynamic icon appears in a content control  (\see CRhRdkContentCtrl)
	};
	...
	/*virtual*/ void SetInstanceName(const wchar_t* wszName, bool bRenameEvents, bool bEnsureNameUnique);
	...
	/*virtual*/ ON_wString DisplayName(void) const;
	...
	/** Reset selected parameters to default values.
		\param aParams is an array of (XML) names of parameters that require resetting.
	...
	/** \return \e true if the content is hidden in persistent lists. */
	...
	/** Set this content as hidden - will not show in persistent lists unless the content filter "Show Hidden Content"
		is checked. This feature is used for "implementation detail" contents - like picture frame objects, or content
		that's cooked up on the fly in response to scripting on V4 style SDK modifications to the material table. */
	...
	*/
	...
	/** Implement this method to get the content's parameters into an XML section.
		...
		\param context gives the reason the method is being called.
		\param reserved is reserved for future use; set to zero.
	...
	static CRhRdkContent* FromXMLStream(const wchar_t* wszXML, const CRhinoDoc* pDocAssoc);
	...
	/** Create an 'instance' of the content tree and group the new tree with this tree.
		...
		\return A grouped instance of the content tree.*/
	...
	/*virtual*/ bool IsChild(void) const; // [SDK_UNFREEZE]
	...
	/** Set the 'on' state of the specified child slot.
		...
		\param cc is the context in which the set operation is being performed.
		\return \e true if successful, else \e false. */
	virtual bool SetChildSlotOn(const wchar_t* wszChildSlotName, bool bOn);

	/** Get the amount of the specified child slot.
	...
	/*virtual*/ ON_wString ChildSlotDisplayName(void) const; // [SDK_UNFREEZE]
	...
	/** Get the render CRC of the content hierarchy. This is an advanced override which is rarely overridden.
		...
		\see Changed() 
		\see RenderCRC(const CRenderCRCFlags& rcrcFlags, const IRhRdkLinearWorkflow& lw) */
	...
	/** Query whether or not the content hierarchy is owned by a document.
		...
		that owns them. However, it is also possible for contents to be 'attached'
		to a document indirectly and not actually owned by the document. For example,
		decal textures which are attached to the document via document-owned decals.
	...
	/** Get the RDK document that this content hierarchy is registered with, if any.
		\note There is a subtle but important difference between 'registered' and 'owned'.
		 The difference is ownership. Owned contents are owned by the document, and they
		 are also registered with it (and attached to it). Registered contents are also
		 'attached' but not necessarily owned by the document. For example, decal textures
		 are owned by document decals but they are 'attached' because they have document
		 registration. Most contents you encounter will either be both document-registered
		 and owned by a document, or neither (probably owned by you). Some may be
		 document-registered but owned by decals.
		\return A pointer to the RDK document that the content hierarchy is registered
		 with or null if the content hierarchy is not registered with a document. */
	virtual const CRhRdkDocument* RdkDocumentRegistered(void) const;
	...
	/** \deprecated This method is not implemented and returns null.
		...
		then use CRhRdkDocument::RhinoDoc() and check for null. */
...
public:
	...
	/*virtual*/ void SetInstanceNameImpl(const wchar_t* wszName, bool bRenameEvents, bool bEnsureNameUnique);
	...
	*/
	...
	/** \internal For RDK internal use only. */
	/*virtual*/ bool OpenInFirstMainModelessEditor(void) const;

	/**	Currently implemented through EVF "GetUnderlyingInstances", this allows a content
		to act as a proxy for a collection of other contents. This method is called when
		getting the current selection for display in the UI. The default implementation
		is to simply return 'this' in the array. Currently only the Texture Proxy overrides
		this to return its 'aggregate'. */
	/*virtual*/ bool GetUnderlyingInstances(OUT CRhRdkContentArray& a) const;
...
public:
	...
	/** Set the array of contents as 'combineable'. Only used by textures. */
	void SetIsCombineable(bool b);
