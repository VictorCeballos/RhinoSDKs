...
public:
	...
	/*virtual*/ ON__UINT64 StartTimeInMilliseconds(void) const; // = 0; [SDK_UNFREEZE] Uses EVF "StartTimeInMilliseconds" for now.
	/*virtual*/ ON__UINT64   EndTimeInMilliseconds(void) const; // = 0; [SDK_UNFREEZE] Uses EVF "EndTimeInMilliseconds" for now.
...
public:
	...
	enum rendering_info
	{
		...
		ri_gamma                 = 3, // Gamma value or 'on' state changed.
		ri_use_linear_workflow   = 4, // Use linear workflow checkbox changed.
		...
		ri_rhino_settings        = 8, // Rhino settings changed (ON_3dmRenderSettings or its user data).
	...
	/** Called after the document skylight settings have been changed.
		...
		\param newSL is an interface which provides the settings after the change.
		\param oldSL is an interface which provides the settings before the change.
		\param pSender is for internal use only. */
	...
	/** Called when rendering finishes.
		\param bSuccessful is \e true if rendering completed sucessfully, else \e false. */
