...
public:
	...
	RhRdkRenderMeshPrimitiveTypes PrimitiveType(void) const;
	...
	/*virtual*/ void SetGeometryEx(const IRhRdkCustomRenderMeshes& crm, int iIndex);
...
public:
	...
	virtual IRhRdkSdkRenderMeshIterator* NewRenderMeshIterator(const ON_Viewport& vp, bool bForceTriMesh=true, bool bQuiet=false) const;

	virtual bool IsRenderInitializing(void) const;

	/** This method is deprecated in favor of the one below. */
	RDK_DEPRECATED static IRhRdkSdkRenderMeshIterator* NewRenderMeshIterator(const UUID& uuidRenderPlugIn, const ON_Viewport& vp, bool bForceTriMesh=true);

	/** \return A new render mesh iterator. The caller shall delete the iterator.
		Any meshes created by the iterator persist in memory for the lifetime of the iterator. 
		Use this version if you are not using the CRhRdkSdkRender class to manage your render pipeline. */
	static IRhRdkSdkRenderMeshIterator* NewRenderMeshIterator(const CRhinoDoc& doc, const UUID& uuidRenderPlugIn, const ON_Viewport& vp, bool bForceTriMesh=true, bool bQuiet=false);
...
public: // Same interface as CRhinoSdkRender.
	...
	virtual BOOL IgnoreRhinoObject(const CRhinoObject* pObject);
	virtual BOOL AddRenderMeshToScene(const CRhinoObject* pObject, const CRhinoMaterial& material, const ON_Mesh* pMesh);
	virtual BOOL AddLightToScene(const CRhinoLight& light);
	virtual BOOL RenderSceneWithNoMeshes();
	virtual BOOL StartRenderingInWindow(CRhinoView* pView, const LPCRECT rect);
	virtual BOOL StartRenderingQuiet(LPSIZE sizeImage);
