...
public:
  ...
  */
  ...
  void EnableDrawLineFromPoint(BOOL bDrawLineFromPoint = true);
  ...
  void PermitOrthoSnap(BOOL bPermitOrthoSnap = true);
  ...
  Parameters:
    ...
    is automatically avaiable in GetPoint.
  Remarks:
  ...
  */
  void PermitFromOption(BOOL bPermitFromOption = true);
  ...
  Parameters:
    ...
    contraint options are automatically avaiable in GetPoint.
  Remarks:
    By default, these built-in constraint options are availble 
  ...
  See Also:
    ...
    CRhinoGetPoint::PermitFromOption
  */
  void PermitConstraintOptions(BOOL bPermitConstraintOptions = true);
  ...
  */
  void EnableCurveSnapTangentBar( 
          BOOL bDrawTangentBarAtSnapPoint = true,
          BOOL bDrawEndPoints = false
          );

  /*
  Description:
    Controls display of the curve snap perpindicular bar icon.
  Parameters:
  ...
  */
  void EnableCurveSnapPerpBar( 
          BOOL bDrawPerpBarAtSnapPoint = true,
          BOOL bDrawEndPoints = false
          );
  ...
  */
  void EnableCurveSnapArrow( 
    BOOL bDrawDirectionArrowAtSnapPoint = true,
    BOOL bReverseArrow = false
    );
  ...
  void EnableSnapToCurves(BOOL=true);
  ...
  */
  ...
  BOOL Constrain( ON_3dPoint, ON_3dPoint ); // constrain to a line
  BOOL Constrain( const ON_Line&, BOOL bTrackFromStart = true );        // constrain to a line
  BOOL Constrain( const ON_Arc& );        // constrain point to lie on an arc
  BOOL Constrain( const ON_Circle& );     // constrain point to lie on a circle
  BOOL Constrain( const ON_Plane&, bool bAllowElevator = false );      // constrain point to lie on a plane
  BOOL Constrain( const ON_Sphere& );     // constrain point to lie on a sphere
  BOOL Constrain( const ON_Cylinder& );   // constrain point to lie on a cylinder
  ...
  BOOL Constrain( const ON_Curve&, bool bAllowPickingPointOffObject = false );
  BOOL Constrain( const ON_Surface&, bool bAllowPickingPointOffObject = false );
  ...
  BOOL Constrain( const ON_Brep&, int wire_density = -1, int face_index = -1, bool bAllowPickingPointOffObject = false );
  BOOL Constrain( const ON_BrepFace&, int wire_density = -1, bool bAllowPickingPointOffObject = false );

  BOOL Constrain( const ON_Mesh&, bool bAllowPickingPointOffObject = false );
  ...
  // See Also:
  //   SetBasePoint.
  BOOL ConstrainToConstructionPlane( BOOL bThroughBasePoint = true, BOOL bProjectOsnaps = false );

  // Constrain point to lie on a plane that is parallel to the
  // viewing plane and passes through the view's target point.
  BOOL ConstrainToTargetPlane(); 
  ...
  // InterruptMouseMove() to see if you should interrrupt
  // your work because the mouse has moved again.
  //
  BOOL InterruptMouseMove() const; // tool for use in 
...
private:
  ...
  BOOL m_bOnPlaneVirtualCPlaneIntersection;
  ...
  BOOL m_bPermitOrtho; // default = true
  ...
  BOOL m_bHaveObjRef; // true if point was on an object
