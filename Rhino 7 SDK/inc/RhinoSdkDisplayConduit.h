...
/*
  const static UINT SC_RESERVED_CHANNEL       = 0x00100000;
...
*/
  ...
  #define CHANNELS_SUPPORTED                    0x000FFFFF
...
public:
  ...
  //   EConduitNotifiers occurs. This function is typically not overriden, but may
...
public:
  ...
  // Returns:
  //   false if this conduit is alread bound to the provided pipeline/viewport
  ...
  //                     neccessary to pay attention to the state since conduits can be manually bound
  ...
  //        ALL geometry will be processed and is equivelant to passing 
  ...
  //    false - if passed in object's type does NOT match against the currenty geometry filter.
  //
  // Note: If the current geometry filter is set to ON::any_object OR 0, this routine will
  //       always return true, and no condtions or comparisons are checked or made.
...
protected:
  ...
  CChannelAttributes*   m_pChannelAttrs = nullptr;
