...
// The Mac version is entirely different
...
// DC.  The HBITMAP and CRhinoDib::FindBits() point to the smae memory.  You can modify the
...
public:
  ...
  */
  ...
  public:
    ...
    Pixel(BITMAPINFO& in_bmi, LPBYTE in_bits, long in_cd, long in_x, long in_y, unsigned char in_r, unsigned char in_g, unsigned char in_b, unsigned char in_alpha)
      :
      bmi(in_bmi), dib_bits(in_bits), color_depth(in_cd), x(in_x), y(in_y), r(in_r), g(in_g), b(in_b), alpha(in_alpha)
    {}
  };
  ...
  // Pixel iterators with an interupt.  Return false in the callback function to interupt the loops.  
  ...
  */
  ...
  public:
    ...
    BOOL GetPixel(long x, long y, int& r, int& g, int& b, int& alpha) const;
    BOOL SetPixel(long x, long y, int r, int g, int b, int alpha = 255);
  ...
  */
  ...
  Read bitmap from disk.
  Parameters:
  lpsFileName - [in] Name of file to read.  Uses CRhinoFileUtilities::FindFile(...) to validate file name.s
  ...
  */
  bool Print(HWND hWnd = nullptr, bool bFitToPage = true);
