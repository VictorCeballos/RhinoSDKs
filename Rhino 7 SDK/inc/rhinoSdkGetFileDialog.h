...
public:
  ...
  enum file_dialog_type
  {
    ...
    file_dialog_type_count = custom, // For backward compatibilty only. DO NOT USE. Please change your code to use 'custom'.
  };
  ...
  /*
  Description:
    This functon is will show just about any kind of Rhino related file
  ...
  */
  ...
  /*
  Returns:
    True if the file is a pervious Rhino version 3dm file.
  */
  ...
  */
  ...
  BOOL SetShowPreview( BOOL);

  int BitmapTypes() const;
  int SetBitmapTypes( int);

  BOOL AddPlugInFileTypes();
  BOOL SetAddPlugInFileTypes( BOOL b);
  ...
  BOOL UseAsDeafultTemplate() const;
  void SetUseAsDefaultTemplate( BOOL b);
  ...
  void SetScriptMode( BOOL bInScriptMode);

  static const wchar_t* GetDefaultDirectory( file_dialog_type ft);
  static BOOL SetDefaultDirectory( file_dialog_type ft, const wchar_t* lps);
...
protected:
  BOOL BuildExtensionList( file_dialog_type);
  ...
  BOOL                         m_bShowPreview;    // Defaults to true
  BOOL                         m_bAddPlugInTypes; // Defaults to true
  BOOL                         m_bUseAsDefault;   // Use as default template
  int                          m_types;           // Defaults to all_types
  CRhinoFileReadOptions        m_fro;
  CRhinoFileWriteOptions       m_fwo;
  CRhinoFileWriteOptions       m_fwo_in;
  BOOL                         m_bUseFileDialogs;
  // 16 October 2012 by John Morse changed this from a BOOL to a ON__UINT32
