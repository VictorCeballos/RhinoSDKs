...
public:
  ...
  //   VerticalConeCyl, EnableVerticalConeCylOption
  void SetVerticalCircle( BOOL bSelected = TRUE);
  ...
  //   VerticalCircle, EnableVerticalCircleOption
  //   VerticalConeCyl, EnableVerticalConeCylOption
  void EnableVerticalCircleOption(BOOL bAvailable = TRUE);
  BOOL VerticalCircleOptionAvailable() const;
...
//  void SetDeformable( BOOL set = TRUE);
...
protected:
  ...
  BOOL m_bUsePoint0;               // [i]  Use first (center) instead of asking for point
  BOOL m_bUsePoint1;               // [i]  Use second instead of asking for point
  BOOL m_bUsePlane;                // [i]  Constrain the circle to a plane
  BOOL m_bVerticalCircleAvailable; // [i]  TRUE if "Vertical" option is available and applies to VerticalCircle
  BOOL m_bVerticalCircle;          // [io]  Make the circle axis parallel to the current construction plane
  bool m_bConeCylAvailable;        // [i]  TRUE if "Vertical" option is available and applies to VerticalConeCyl
  bool m_bCap;                     // [io] Used when RhinoGetCircle is called by a primitive solid command like cylinder or cone
  bool m_bReserved1;
  bool m_bReserved2;
  int m_ConeCylConstraint;          // [io]  0 = none, 1 = vertical, 2 = aroundcurve
  BOOL m_bUseDefaultSize;          // [i]  Defaultsize is valid
  BOOL m_bUseDiameterMode;         // [io] Get size in diameter mode instead of radius
  BOOL m_bDynamicDrawSphere;       // [i]
  eCircleResult m_result;         // [o]  What mode was used

  BOOL m_allow_deformable;         // display deformable prompt or not
  BOOL m_deformable;               // make circle deformable or not
