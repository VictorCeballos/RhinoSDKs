...
RHRDK_SDK extern UUID uuidData_RdkDithering;                     // Return value must be cast to IRhRdkDithering*
...
RHRDK_SDK extern UUID uuidData_RdkGroundPlane;                   // Return value must be cast to IRhRdkGroundPlane*
RHRDK_SDK extern UUID uuidData_RdkImageAdjust;                   // Return value must be cast to IRhRdkImageAdjust*
RHRDK_SDK extern UUID uuidData_RdkImageFile;                     // Return value must be cast to IRhRdkImageFile*
...
RHRDK_SDK extern UUID uuidData_RdkLinearWorkflow;                // Return value must be cast to IRhRdkLinearWorkflow*
...
RHRDK_SDK extern UUID uuidData_RdkRenderChannels;                // Return value must be cast to IRhRdkRenderChannelsDS*
...
RHRDK_SDK extern UUID uuidData_RdkRenderingGamma;                // OBSOLETE.
RHRDK_SDK extern UUID uuidData_RdkRenderingToneMapping;          // OBSOLETE.
RHRDK_SDK extern UUID uuidData_RdkToneMapping;                   // OBSOLETE.
RHRDK_SDK extern UUID uuidData_RdkRhinoSettings;                 // Return value must be cast to IRhRdkRhinoSettings*
RHRDK_SDK extern UUID uuidData_RdkSafeFrame;                     // Return value must be cast to IRhRdkSafeFrame*
RHRDK_SDK extern UUID uuidData_RdkSelectionNavigator;            // Return value must be cast to IRhRdkSelectionNavigator*
RHRDK_SDK extern UUID uuidData_RdkSkylight;                      // Return value must be cast to IRhRdkSkylight*
RHRDK_SDK extern UUID uuidData_RdkSun;                           // Return value must be cast to IRhRdkSun*
...
RHRDK_SDK extern UUID uuidData_RdkTinyIcons;                     // Return value must be cast to IRhRdkTinyIcons*
...
/** Note that if you want to support the Linear Workflow settings in Rhino, you will need to do the following in your renderer:
	...
	1) Return \e true for uuidFeatureLinearWorkflow in your CRhRdkPlugIn::SupportsFeature override.

	2) Check that Linear Workflow is on using IRhRdkLinearWorkflow::On(), and if so:

	3) Convert all colors that you get from Rhino to their linear equivalent using
	   CRhRdkColor::ApplyLinearWorkflowGamma() / IRhRdkLinearWorkflow::ApplyLinearWorkflowGamma()

	4) Convert all texture colors *excluding HDR textures* (those that return true for CRhRdkTexture::IsLinear), or textures
	   that are used in a bump or normal map using CRhRdkColor::ApplyLinearWorkflowGamma().
	  \note You can call CRhRdkMaterial::TextureRequiresLinearWorkflowAdjustment() to determine if the texture requires it.

	5) If you are not using the built-in render window, apply the gamma value in IRhRdkLinearWorkflow::Gamma().
	   \note While Rhino 5 automatically did some of the gamma correction work in SimulateMaterial(), this is not the case
	   in Rhino 6. Everything now has to be done explicitly by the client.
*/
...
RHRDK_SDK extern UUID uuidTaskFloatingPreview;
...
// Render window tabs.
RHRDK_SDK extern UUID uuidExposureRenderWindowTab;
RHRDK_SDK extern UUID uuidPostEffectRenderWindowTab;
