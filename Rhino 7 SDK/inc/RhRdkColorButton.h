...
*/
class RHRDK_SDK IRhRdkColorButton : public IRhinoUiWithController
{
public:
...
*/
class RHRDK_SDK CRhRdkColorButton : public CWnd
{
public:
	CRhRdkColorButton();
...
/** While a color picker is being displayed, this function returns a pointer
	to the color button that launched the picker. It is the only safe way
	to access the color button from a color picker. You must call it every time
	you need the pointer; do not store the pointer returned by this function. */
RHRDK_SDK IRhRdkColorButton* RhRdkGetActiveColorButton(void);
