...
public:
	...
	/** Get values from a section into fields with the same internal name.
		...
		\param cc is the context of the change which requires this operation.
...
public:
	...
	RDK_DEPRECATED void SetTextureAmount(int iAmount);

	/** \return The texture amount as a double. The typical range is 0.0 to 100.0 */
	double TextureDoubleAmount(void) const;

	/** Set the texture amount. The typical range is 0.0 to 100.0 */
	...
	RDK_DEPRECATED void SetTextureAmountMin(int iAmountMin);
	...
	RDK_DEPRECATED void SetTextureAmountMax(int iAmountMax);
