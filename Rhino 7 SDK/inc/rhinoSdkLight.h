...
public:
  ...
  ON_BoundingBox BoundingBox( const CRhinoViewport* pViewport ) const;
  ...
  int Pick( const CRhinoPickContext&, CRhinoObjRefArray& ) const;

  // virtual CRhinoObject::SnapTo override
  bool SnapTo( const CRhinoSnapContext& snap_context, CRhinoSnapEvent& snap_event) const;
...
public:
  ...
  //
  // Returns:
  //   Refererence to the light.  If light_index is out of range,
  ...
  Example:
          ...
          BOOL bIgnoreDeletedLights = TRUE;
  ...
  */
  ...
  //       BOOL cull(const CRhinoLight* arg,void* p).
...
private:
  ...
  BOOL IndexInRange( int light_index, const wchar_t* lpsMessageTitle = 0);
