...
public:
  ...
  BOOL m_bClosed;                 // Open or closed sweep if the rail is closed
  ...
  int m_miter_count;              // number of kink miters sucessfully complered
...
public:
  ...
  BOOL m_bSimpleSweep;       // Make a simple surface by greville interpolation of shapes
  BOOL m_bClosed;            // Open or closed sweep if both rails are closed
  int m_simplify;            // Simplify method for shape curves
  int m_rebuild_count;       // Sample point count for rebuilding shapes
  double m_refit_tolerance;  // tolerance to refit shape curves
  double m_sweep_tolerance;  // tolerance to pass to TL sweeper as SweepTolerance
  ...
  BOOL m_bSameHeight;        // maintain height between shapes - don't scale it the same as width
  void* m_pUser;
private:
  ON__UINT_PTR m_sdk_reserved = 0;
};
...
RHINO_SDK_FUNCTION
BOOL RhinoSweep1( CArgsRhinoSweep1& args, ON_SimpleArray<ON_Brep*>& brep_array);

RHINO_SDK_FUNCTION
BOOL RhinoSweep2( CArgsRhinoSweep2& args, ON_SimpleArray<ON_Brep*>& brep_array);
...
*/
RHINO_SDK_FUNCTION
BOOL RhinoSweep1Ex( CArgsRhinoSweep1& args, int shape_blending, ON_SimpleArray<ON_Brep*>& brep_array );
...
Parameters:
  ...
  bFrameStyle  [in] - Determines how sweep frame rotations are calculated:
                      true: Freeform - Frames are propogated based on a refrence direction taken from the rail curve curvature direction.
