...
public:
	...
	enum eDrawNormal { dn_none, dn_forward, dn_backward, dn_both }; // Direction for AddPlaneToDrawCache. Draws 'normal' arrow(s).
...
public:
	...
	virtual int GripOriginIndex(void) const { return 0; }

	/** The grip at (count - index) is usually the opposite one.
		Optionally override this method to return a different index. */
	virtual int GripOppositeIndex(int index, int count) const { return count - index; }
