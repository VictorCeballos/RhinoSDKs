...
public:
  ...
  */
  ...
  // Parameters:
  //   dib - [out] 24 Bit color DIB contaning screen capture of this view
...
public:
  ...
  void OnMouseDownSelectObject( 
          int button,
          BOOL bCtrl,  // true means Ctrl key is down
          BOOL bShift, // true means Shift key is down
          BOOL bAlt, // true means Alt key is down
...
public:
  ...
  //
  //Parameters:
  //  bFloat [in] If true then the view will be embeded in a floating frame window otherwise
...
class RHINO_SDK_CLASS CRhinoViewIterator
{
  ...
  Example:
    Iteratate all views:
    unsinged int rhino_doc_sn = 0 or doc->RuntimeSerialNumber();
