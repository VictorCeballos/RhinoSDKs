...
public:
  ...
  */
  void EnableOneByOnePostSelect(
    BOOL bEnableOneByOnePostSelect = true
    );
  ...
  */
  void EnableSubObjectSelect(
          BOOL bEnableSubObjectSelect=true
          );
  ...
  */
  ...
  Description:
    ...
    prefered.  (polysurface beats face beats edge).  If you
    want the bottom most object to be prefered, then call 
  ...
  */
  ...
  Parameters:
    bEnable - [in]
      ...
      false -
        Selection of SubD objects and SubD components is prohibited. This is useful in commmands that
  ...
  */
  ...
  void EnableSelPrev(BOOL=true); // default is true
  ...
  void EnableHighlight(BOOL=true); // default is true
  ...
  */
  ...
  /*
  Returns:
    True if CRhinoGetObjects::GetObjects() retuned preselected objects.
  */
  ...
  */
  ...
  Remarks:
    If you pass false to this fuction, then you are
...
Parameters:
  view       - [in] view where ambiguous pick occured
  view_point - [in] window coordinates where ambiguous pick occured.
  ...
  visibility_hint - [in] By default, in a shaded view point pick,
     ...
     candidates.  If objects are were visible when the pick occured
     but are no longer visible, then pass them in here.  For example,
     if a wire frame object was selected when the pick occured but
...
public:
  ...
  CRhinoMeshRef(CRhinoObject*, ON_Mesh*, 
            BOOL // true if geometry is a proxy object that needs to be deleted
            );
  ~CRhinoMeshRef();
...
public:
  ...
  BOOL GetExportMeshes(
         BOOL=false // set to true to mesh only selected objects
         );
