...
public:
  ...
  CRhinoUiGridListEdit *GetEditCtrl();
	...
	void Setup( BOOL bFullRowSelect, BOOL bEditOnClick = TRUE, BOOL bListChange = TRUE);
	// This function is called after editing is completed, if you don't
	// override it then the list item text is updated.
	virtual BOOL DoneEditing( long nItem, int nSubItem, LPCTSTR lpszText);
	...
	virtual BOOL ModifyGridItem( long nItem, int nSubItem); 
  ...
  void DestroyEditCtrl();
	...
	// text has changed.
	BOOL       EditGridItem( long nItem, int nSubItem);
	// Display combo box in row and column specified.  ComboBox will contain
	// lstItems and will default to nSel.  DoneEditing is calle when user selects
	...
	BOOL NotifyOnListChange(){ return m_bListChange; }
	void NotifyOnListChange( BOOL b){ m_bListChange = b; }
	// Determines if items will be edited on a single or double click.  Single is
	// default.
	BOOL EditOnClick(){ return m_bEditOnClick; }
	void EditOnClick( BOOL b){ m_bEditOnClick = b; }
...
private:
  ...
  enum ScrollSpeeds
  {
	...
	BOOL m_nEdit;              // Column current edit item is in
	BOOL m_bEditOnClick;       // Edit on a single click
	BOOL m_bListChange;        // Call DoneEditing only if ListBox changes
	                           // default is TRUE
	CRhinoUiGridListEdit m_LVEdit; // Edit control used to SubClass
	                           // default CListCtrl edit control.
  bool       m_bAllowColumnResizing;     // If true then columns may be resized, defults to true.
