...
#if defined (ON_RUNTIME_APPLE)
  CRhinoDisplayPipeline(CRhinoViewport& vp, CRhinoDoc* pDoc);
#endif
...
public:
  ...
  UINT GetFrameTick() const      { return ::clock(); }
  ...
  UINT GetFrameTickDelta() const { return ::clock() - m_nFrameStartTime; }
  ...
  //  >0 nesting level of active object and lenght of "stack"
...
public:
  ...
  // Parameters:
  //   surface      : [in]
  //   wire_density : [in] Density of iso-curves
  ...
  */
  ...
  //       process.
  void Draw2dRectangle(int x0, int y0, int x1, int y1, ON_Color strokeColor, float strokeWidth, RhinoDashStyle style, bool _xor);
  void Draw2dRectangle(float x0, float y0, float x1, float y1, ON_Color strokeColor, float strokeWidth, RhinoDashStyle style, bool _xor);
  ...
  void Draw2dLine(int x0, int y0, int x1, int y1, ON_Color strokeColor, float strokeWidth, RhinoDashStyle style, bool _xor);
  void Draw2dLine(float x0, float y0, float x1, float y1, ON_Color strokeColor, float strokeWidth, RhinoDashStyle style, bool _xor);
  ...
  //                       cordinates where the top is is number of pixels from the top of the viewport
...
public:
  ...
  //        while "inside" the pipeline. If called inside the pipleine,
  ...
  //                              It is the caller's responsibilty to free
...
protected:
  // Description:
  //  Called before an object is drawn. This function is resposible for determining if the object
  ...
  //  false if an error occured or the object is determined to not be visible
