...
/*
Description:
  RhinoMeshSplit divides the meshes in meshesToSplit with meshesThatSplit.  That is, each mesh in meshesToSplit is
  split, singularly, with all the meshes in meshesThatSplit at the same time.
  If the attribute arrays are non-null, then the attributes for meshesToSplit will be provided.
  This function only uses the new intersector.

Parameters:
  meshesToSplit          - [In]  Meshes to be split
  meshesThatSplit        - [In]  Meshes to split with
  intersectionTolerance  - [In]  General tolerance to be used in mesh-mesh intersection. Values in the range of 10e-7 are good guesses for meshes in the 100 range.
  preprocessing          - [In]  When intersectionTolerance is smaller than the distance between vertices and edges, this allows for a preprocess step to try to 
                                 improve splitting outcomes.
  splitAtCoplanar        - [In]  If false, coplanar areas will not be split.
  somethingHappened      - [Out] Set to true if something actually happened, independent of function return
  outMeshes              - [Out] Result of the split of the input meshes
  inputAttributeArray    - [In]  Optional array of attributes.  Must be not be null and have count equal to InMeshes to be used.
  ouputAttributeArray    - [Out] Optional array of attributes.  Must be not be null to be used.
  textLog                - [In]  A log to write onto. Can be a CRhinoDump reference, or nullptr.
  terminator             - [In]  A termination token, or nullptr.
  reporter               - [In]  A progress reporter, or nullptr.

Returns:
  True or false based on whether the function was able to complete successfully, also because of a terminator being raised.
  It does not indicate whether the function actually split the input meshes.  The argument, somethingHappened, is used for that purpose.

See Also:
  RhinoMeshBooleanDifference, RhinoMeshBooleanUnion, RhinoMeshBooleanIntersection, RhinoMeshBooleanSplit
*/
RHINO_SDK_FUNCTION
bool RhinoMeshSplit(
  const ON_SimpleArray<const ON_Mesh*>& meshesToSplit,
  ...
  const ON_SimpleArray<const ON_3dmObjectAttributes*>* inputAttributeArray = 0,
  ON_SimpleArray<const ON_3dmObjectAttributes*>* ouputAttributeArray = 0,
  ON_TextLog* textLog = 0,
  ON_Terminator* terminator = 0,
  ON_ProgressReporter* reporter = 0);
...
Description:
  ...
  of "unwelded" edges.  Unwelded edges are edges where the faces that share the edge have unique mesh vertexes (not mesh topology vertexes) at both 
...
Description:
  ...
  than or equal to angle_tol have unique vertexes along that edge, adding vertexes if necessary.
  
Parameters:
  OldMesh      - [In]  Mesh to be unwelded
  angle_tol    - [In]  Angle at which to make unique vertexes as decribed above
...
Description:
  ...
  than or equal to angle_tol share vertexes along that edge, vertex normals are averaged.
...
/*
Description:
Moves mesh vertexes that belong to naked edges to neighboring vertexes within the specified distance.

Parameters:
MeshArray - [in/out] Input array of meshes.  These meshes are modified.
VertexFlags - [in] Array of arrays of flags to determine which vertexes may be modified.
distance - [in] Distance to not exceed when modifying the meshes.

Returns:
Count of all vertexes that were modified if successful. -1 for error.
*/
RHINO_SDK_FUNCTION
...
public:
  ...
  // If m_bCheckForDuplicateFaces is true then the function will look for faces that have vertexes (not necessarily vertex indexes) that are identical.
  ...
  // If m_bCheckForUnusedVertices is true then the function will look for vertexes that are not referenced by any face in the mesh.  
...
private:
  ...
  // Values below are reseverd for future use.
...
Parameters:
  ...
  text_log      - [Out]     ON_TextLog with printed output of what was discovered based on the things set to check in the ouput parameter, 
...
/*
Description:
  RhinoExtractMeshFaces extracts mesh faces using list of indeces
  and return two meshes.
  
Parameters:
  FaceIDXArray - [in] input list of face indeces to be extracted
...
public:
  ...
  int m_iFaceLimit; // Runs a mesh reduction as a post process o simplify the result of displacement
...
Parameters:
A, B, C, D - [In] 4 vertexes of a mesh face, face_points[2] and face_points[3] are identical for triangles
