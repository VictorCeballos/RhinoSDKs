...
public:
	...
	/** Adjust the color for linear workflow gamma. This method is deprecated in favor of the other one which is faster.
	...
	/** Adjust the color for linear workflow gamma.
		If linear workflow is enabled, color is adjusted and the function returns \e true. Otherwise the function returns \e false. */
	bool ApplyLinearWorkflowGamma(const class IRhRdkLinearWorkflow& rdkDoc, bool bForTexture);
