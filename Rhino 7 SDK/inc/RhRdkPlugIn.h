...
public:
	...
	/** Optionally override this method to register extensions whose interface is defined by
		...
		\see CRhRdkExtension \see AddExtension() \see RhRdkAddExtension */
	...
	/** Adds an extension to the RDK. If you override this method, please be sure to call the base class.
		...
		\see CRhRdkExtension \see RegisterExtensions() \see RhRdkAddExtension */
...
public:
	...
	/** \return plug-in type. */
	virtual ePlugInType PlugInType(void) const { return rdkRenderPlugIn; }
	...
	/** You can optionally override this method to add custom sections to the Render Settings dialog.
		...
		\param aDialogs is the array to append your custom dialogs to.
	...
	/*virtual*/ void SupportedChannels(OUT ON_SimpleArray<UUID>& aChan) const; // [SDK_UNFREEZE]
	...
	/*virtual*/ ON_wString CustomChannelName(const UUID& uuidChan) const; // [SDK_UNFREEZE]
...
/** Add an extension to the RDK.
	...
	\see CRhRdkExtension. \see CRhRdkPlugIn::RegisterExtensions(). \see CRhRdkPlugIn::AddExtension(). */
...
/** Queries whether or not the Safe Frame is visible for a certain document.
	\return \e true if visible, else \e false. */
RHRDK_SDK bool RhRdkIsSafeFrameVisible(CRhinoDoc& doc);
...
/** Queries whether or not the Ground Plane is visible for a certain document.
	\return \e true if visible, else \e false. */
RHRDK_SDK bool RhRdkIsGroundPlaneVisible(CRhinoDoc& doc);
...
/** Get an interface to an automatic UI. The caller shall delete the interface
	...
	\param pParent is the parent window which <b>must not be nullptr</b>.
...
/** This method is deprecated in favor of CRhRdkSdkContentSelector::GetSelection(). */
...
/** This is deprecated because HICON can't appear in the RDK SDK. */
RDK_DEPRECATED RHRDK_SDK bool RhRdkConvertIconToDib(HICON hIcon, CRhinoDib& dibOut);

/** This is deprecated because HICON can't appear in the RDK SDK. */
RDK_DEPRECATED RHRDK_SDK HICON RhRdkConvertDibToIcon(const CRhinoDib& dib);
...
{
public:
	CRhRdkSdkContentSelector(const UUID& uuidTarget, const CRhinoDoc& doc, UINT_PTR uReserved=0);
	virtual ~CRhRdkSdkContentSelector();

	virtual bool UnselectAllContent(void) const;
	virtual bool GetSelection(CRhRdkInstanceIdArray& aContentOut) const;
...
RHRDK_SDK bool RhRdkGetUnpackTargetFolder(ON_wString& sTargetFolderOut);
 ...
 * developer specific support to the basic Rhino 6.0 SDK.  The RDK includes support for a fully extensible
 ...
 *
 * Please see the file folder "Marmalade" in the c:\\program files\\Rhino 4.0 SDK install folder - 
 * this is a complete implementation of a rendering plug-in including automatic UI materials, frame
 * buffer support.
 */
