...
public:
  ...
  bool DrawThemeText( HDC hdc, int iPartId, int iStateId, LPCTSTR pszText, int iCount, DWORD dwTextFlags, BOOL bGrayedOut, const RECT *pRect);
  bool DrawThemeText( HDC hdc, int iPartId, int iStateId, const CString& str, DWORD dwTextFlags, BOOL bGrayedOut, const RECT *pRect);
...
private:
  ...
  typedef HRESULT (__stdcall *PFNGETTHEMEBOOL)(HTHEME hTheme, int iPartId, int iStateId, int iPropId, OUT BOOL *pfVal);
  ...
  typedef BOOL (__stdcall *PISTHEMEBACKGROUNDPARTIALLYTRANSPARENT)( HTHEME hTheme, int iPartId, int iStateId);
