...
public:
  ...
  */
  ...
  ON_Line m_M; // smallest chord that contains m_L and such that
               // every point on C projects orthoganally
...
public:
  ...
  Parameters:
    ...
    side [in] If t lies at the start/end of a domain, side
       contols which leaf is returned.  If side >= 0,
  ...
  */
  ...
  // covers the portion of the curve from m_domin.Min() to m_domain.Mid().
  ...
  */
  ...
  Description:
    Quickly determine if the shortest distance from
    the point P to the portion of the curve coverered by
  ...
  */
  ...
  // with beginning, (or ending respecively) at t0 when side =0 or 1 ( or side = -1 ).
  // Returns nullptr if range is not contained the domain of this subtree.
  ...
  Parameters:
    s - [in] normalized (bezier parameter) 0 < s < 1
             The cooresponding surface domain parameter is
  ...
  */
  ...
  // Memory managment accounting information
...
public:
  ...
  */
  ...
  /*
  Description:
    Evaluate the portion of the surfac covered by this
    node.
  Parameters:
    s - [in]
    ...
    v - [out] array of length (der_count+1)*v_stride
        ...
        Ds is retuned in (v[v_stride],...,v[v_stride+m_dim-1]),
        Dt is retuned in (v[2*v_stride],...,v[2*v_stride+m_dim-1]),
        Dss is retuned in (v[3*v_stride],...),
        Dst is retuned in (v[4*v_stride],...),
        Dtt is retuned in (v[5*v_stride],...),
        etc.
    quadrant - [in] optional - 1,2,3 or 4
  ...
  */
  ...
  /*
  Description:
    Find the smallest node that contains the specfied paramter range.
  Parameters:
    Urange - [in]
    Vrange - [in]
  Returns:
  ...
  */
  ...
  unsigned char m_bezmem;    // 0 = do not destroy
                             // 1 = destroy with delete
                             // 2 = destroy with free
  unsigned char m_reserved_ON_SurfaceTreeNode[5]; // keep sizeof() a multple of 16
...
public:
  ...
  Parameters:
    ...
    hit - [in/out]
      ...
      start of the ray.
  Example:
    Shoot a ray at a collecion of "mirrors" and let it reflect.
...
Description:
  ...
  to a surface at a givent parameter.
Parameters:
  srf - [in] surface
...
Description:
  ...
  derivative, and calculated thirdd derivative
...
ON_DECL
/*
  This function intesects two meshes.
  Consider using the more advanced methods of the ON_Mesh classs: IntersectArray, GetSelfIntersections, etc...
*/
...
//   mesh [in] - a mesh.
//   ray [in] - the ray.
//   tolerance [in] - a small, positive value that is used to tell ovelaps and perforations apart.
...
//    consitently.  PC_53 is also Microsoft's default setting.
...
/*
Description:
  Set the FPU control bits to the Microsoft's prefered defaults.
...
Description:
  ...
  so we don't loose any information about excecptions in our
...
Paramters:
  ...
  bCall_ON_Error - [in]
    If true and and exception has occured or
    the control settings have been changed,
    then ON_Error() is called.
Returns:
  true:
    The FPU status indicates no exceptions have occured and the
    control settings match the expected values.
  false:
    The FPU status indicates exceptions have occured or the
...
public:
  ...
  // distance interesects the clashing elements.
  ON_3dPoint m_P;
};
...
public:
  ...
  bool Minimize(const double* seed, //Input.  Dim of them
    ...
    bool* bError //Output.  If true, then there was a serios
