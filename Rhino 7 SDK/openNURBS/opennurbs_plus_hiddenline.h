...
public:
	...
	Details:
		...
		Another option is to leave the target_depth unset, or more explicity call
		  v.SetTargetPoint( ON_UNSET_POINT)
		When the taget point is unset, a target_depth will be computed from the geometry and used to define
		the HLD coordintate system.  
	...
	*/
	...
	Parameters:
		...
		CameraUp -[in]        Direction, in world corrdinates,  that corresponds to increasing y-direction 
	...
	*/
	...
	Remarks:
		...
		For perspective views SetViewport() will add 6 clipping planes from the viewport,
	...
	*/
	...
	Parameters:
		...
		xform - [in] If not null this transformation is applied to geom to place it in the world coordinte system.
	...
	Details:
		...
		geometry persists untill the  ON_HiddenLineDrawing is destroyed.
	*/
	...
	*/
	...
	// Flatten()  projects all m_full_curve to the x-y plane in HLD-coordninates.
...
protected:
	...
	bool m_bUseXform;			// if true use xform.  This allows a simple test instead always testing for th
};
...
public:
	...
	enum class VISIBILITY : unsigned int
	{
		...
		/* Deprecated : kClipped is not actually generated by HiddenLineDarawings and should not be used*/
		kClipped = 5					// clipped by clipping planes
	};

	VISIBILITY m_vis = VISIBILITY::kUnset;

	bool IsSceneSilhouette() const;		// Is this Curve a scene silhoutte.
  ...
  // True if this curve on the boundary of the visible region, as defined by the clipping planes.
	...
	// With respect to the make2d image the region to the left ( or right respecively) of this curve is
	// described by m_SilSide[0] ( or m_SideSil[1] respectively).  If exactly one of these regions is empty 
	// this is a scene silhouette.  If this region conains a surface it is either a surface which in 3d is adjacent 
