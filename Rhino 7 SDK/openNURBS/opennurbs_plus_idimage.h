...
class ON_CLASS ON_DepthImage
{
  ...
  */
  ...
  enum CLIP_STATUS : unsigned int
  {
    ...
    frustum_clip_mask = 0x1FF, // (degerate | infrus | xyz_mask )
    ...
    tridir_none = 0x2000, // projected triangle has negligable area (side facing or degenerate)
  ...
  Returns:
    ...
    ON_DepthImage::VISIBILITY::not_visible
      ...
      The convext hull has portions that are visible and not visible.
  ...
  */
  ...
  Parameters:
    depth_test - [in]
      specifes the type of depth test.
    incoming_depth - [in]
      depth value to test.
  ...
  */
  ...
  Returns:
    false
      The image is not ready for drawing or the shader
      intentionall terminated rendering by returning false.
    true
      Otherwise.
  */
  bool DrawBrep(
  ...
  */
  ...
  Parameters:
    ...
    pixel - [out]
      ...
      discoverered where the triangle_mesh is visible.
  ...
  */
  ...
  Returns:
    number of points appened to points[].
  */
...
private:
  ...
  // to convert clipping z coodinates into world depth values.  Clipping coordinates
  ...
  unsigned char m_vp_status = 0;    // 0 unitialized, 1 valid
  unsigned char m_depth_status = 0; // 0 unitialized, 1 erased, 2 depths vary
  unsigned char m_id_status = 0;    // 0 unitialized, 1 erased, 2 ids vary
  unsigned char m_image_status = 0; // 0 unitialized, 1 ready for drawing
...
public:
  ...
  // that belongs to a triangle that may be visisble.
