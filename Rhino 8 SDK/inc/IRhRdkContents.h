...
public:
	...
	/** Replace a content in the document.
		...
		\return \e true if successful, else \e false. */
	/*virtual*/ bool Replace(const CRhRdkContent& oldContent, CRhRdkContent& newContent, bool bPreserve=true); // [SDK_UNFREEZE]
	...
	/** Selects contents in all capable editors that are displaying those contents on behalf of the document.
		...
		\return \e true if successful, else \e false. */
	/*virtual*/ bool SelectContent(const CRhRdkInstanceIdArray& aInstance, bool bAppendToSelection) const; // = 0; [SDK_UNFREEZE] Implemented via EVF(L"SelectContent").
